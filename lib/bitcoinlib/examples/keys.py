...
print(f"Private key     {k.wif()}")
print(f"Private key hex {k.private_hex} ")
print("Compressed      %s\n" % k.compressed)
...
print(f"Private key     {k.wif()}")
print(f"Encrypted pk    {k.bip38_encrypt('TestingOneTwoThree')} ")
...
print(f"Random BIP32 HD Key on testnet {hdk.wif()}")

print("\n=== Import HD Key from seed ===")
k = HDKey.from_seed('000102030405060708090a0b0c0d0e0f')
print(f"HD Key WIF for seed 000102030405060708090a0b0c0d0e0f:  {k.wif()}")
print(f"Key type is : {k.key_type}")
...
print(
    f"HD Key WIF for Private Key L5fbTtqEKPK6zeuCBivnQ8FALMEq6ZApD7wkHZoMUsBWcktBev73:  {k.wif()}"
)
print(f"Key type is : {k.key_type}")

print("\n=== Derive path with Child Key derivation ===")
print("Derive path path 'm/0H/1':")
print(f"  Private Extended WIF: {k.subkey_for_path('m/0H/1').wif()}")
...
if pub_with_privparent != pub_with_pubparent:
    ...
    print(f"{pub_with_pubparent} == {pub_with_privparent}")
...
k = HDKey(wif)
print(f"Witness type derived from wif {wif} is segwit ({k.witness_type})")
print(f"Encoding derived from wif is bech32 ({k.encoding})")
print(f"Segwit bech32 encoded address is {k.address()}")
...
print(f"Encoding derived from wif is base58 ({k.encoding})")
print(f"Segwit P2SH base58 encoded address is {k.address()}")
