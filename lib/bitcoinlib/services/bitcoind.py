...
class BitcoindClient(BaseClient):
    ...
    def from_config(configfile=None, network='bitcoin'):
        ...
        config_fn = 'bitcoin-testnet.conf' if network == 'testnet' else 'bitcoin.conf'
        cfn = None
        if not configfile:
        ...
        port = 18332 if network == 'testnet' else 8332
    ...
    def __init__(self, network='bitcoin', base_url='', denominator=100000000, *args):
        ...
        _logger.info(f"Connect to bitcoind on {base_url}")
    ...
    def getutxos(self, address, after_txid='', max_txs=MAX_TRANSACTIONS):
        ...
        if not res['ismine'] and not res['iswatchonly']:
            raise ClientError(
                f"Address {address} not found in bitcoind wallet, use 'importaddress' to add address to wallet."
            )
    ...
    def estimatefee(self, blocks):
        ...
        try:
            ...
            _logger.info(f"bitcoind error: {e}, {pres}")
