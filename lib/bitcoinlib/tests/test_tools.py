...
db_uris = ((f'sqlite:///{SQLITE_DATABASE_FILE}', init_sqlite), )
if UNITTESTS_FULL_DATABASE_TEST:
    db_uris += (
        (f'mysql://root@localhost:3306/{DATABASE_NAME}', init_mysql),
        (
...
class TestToolsCommandLineWallet(unittest.TestCase):
    ...
    def test_tools_clw_create_wallet(self):
        cmd_wlt_create = f'{self.python_executable} {self.clw_executable} test --passphrase "emotion camp sponsor curious bacon squeeze bean world actual chicken obscure spray" -r -d {self.DATABASE_URI}'
        output_wlt_create = "14guS7uQpEbgf1e8TDo1zTEURJW3NGPc9E"
        output_wlt_delete = "Wallet test has been removed"

        cmd_wlt_delete = f"{self.python_executable} {self.clw_executable} test --wallet-remove -d {self.DATABASE_URI}"
    ...
    def test_tools_clw_create_multisig_wallet(self):
        ...
        cmd_wlt_create = f"{self.python_executable} {self.clw_executable} testms -m 2 2 {' '.join(key_list)} -r -n testnet -d {self.DATABASE_URI}"
        cmd_wlt_delete = f"{self.python_executable} {self.clw_executable} testms --wallet-remove -d {self.DATABASE_URI}"
    ...
    def test_tools_clw_create_multisig_wallet_one_key(self):
        ...
        cmd_wlt_create = f"{self.python_executable} {self.clw_executable} testms1 -m 2 2 {' '.join(key_list)} -r -n testnet -d {self.DATABASE_URI}"
        cmd_wlt_delete = f"{self.python_executable} {self.clw_executable} testms1 --wallet-remove -d {self.DATABASE_URI}"
    ...
    def test_tools_clw_create_multisig_wallet_error(self):
        cmd_wlt_create = f"{self.python_executable} {self.clw_executable} testms2 -m 2 a -d {self.DATABASE_URI}"
    ...
    def test_tools_clw_transaction_with_script(self):
        cmd_wlt_create = f'{self.python_executable} {self.clw_executable} test2 --passphrase "emotion camp sponsor curious bacon squeeze bean world actual chicken obscure spray" -r -n bitcoinlib_test -d {self.DATABASE_URI}'
        cmd_wlt_update = f"{self.python_executable} {self.clw_executable} test2 -d {self.DATABASE_URI}"
        cmd_wlt_transaction = f"{self.python_executable} {self.clw_executable} test2 -d {self.DATABASE_URI} -t 21HVXMEdxdgjNzgfERhPwX4okXZ8WijHkvu 50000000 -f 100000 -p"
        ...
        cmd_wlt_delete = f"{self.python_executable} {self.clw_executable} test2 --wallet-remove -d {self.DATABASE_URI}"
        process = Popen(cmd_wlt_create, stdin=PIPE, stdout=PIPE, shell=True)
        poutput = process.communicate(input=b'y')
        self.assertIn(output_wlt_create, normalize_string(poutput[0]))
    ...
    def test_tools_clw_create_litecoin_segwit_wallet(self):
        cmd_wlt_create = f'{self.python_executable} {self.clw_executable} ltcsw --passphrase "lounge chief tip frog camera build trouble write end sword order share" -r -d {self.DATABASE_URI} -y segwit -n litecoin'
        output_wlt_create = "ltc1qgc7c2z56rr4lftg0fr8tgh2vknqc3yuydedu6m"
        output_wlt_delete = "Wallet ltcsw has been removed"

        cmd_wlt_delete = f"{self.python_executable} {self.clw_executable} ltcsw --wallet-remove -d {self.DATABASE_URI}"
        process = Popen(cmd_wlt_create, stdin=PIPE, stdout=PIPE, shell=True)
        poutput = process.communicate(input=b'y')
        self.assertIn(output_wlt_create, normalize_string(poutput[0]))
        process = Popen(cmd_wlt_delete, stdin=PIPE, stdout=PIPE, shell=True)
    ...
    def test_tools_clw_create_multisig_wallet_p2sh_segwit(self):
        ...
        cmd_wlt_create = f"{self.python_executable} {self.clw_executable} testms-p2sh-segwit -m 3 2 {' '.join(key_list)} -r -y p2sh-segwit -d {self.DATABASE_URI}"
        cmd_wlt_delete = f"{self.python_executable} {self.clw_executable} testms-p2sh-segwit --wallet-remove -d {self.DATABASE_URI}"
